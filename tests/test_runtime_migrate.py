...
def test_migrate_newsletter_moves_runtime_files_and_preserves_settings(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in ("newsletter-digest.sqlite3", "newsletter-digest.sqlite3-wal", "newsletter-digest.sqlite3-shm"):
        (legacy_data / name).write_text(name, encoding="utf-8")
    (legacy_data / "newsletter-digest.lock").write_text("lock", encoding="utf-8")
    ...
    assert (target_data / "2much2read.sqlite3-wal").is_file()
    assert (target_data / "2much2read.sqlite3-shm").is_file()
...
def test_migrate_calendar_moves_distinct_explicit_client_secret(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in ("google-calendar-token.json", "reminders.yaml"):
        (legacy_config / name).write_text(name, encoding="utf-8")
    for name in ("2busy1miss.sqlite3", "2busy1miss.sqlite3-wal", "2busy1miss.sqlite3-shm"):
        (legacy_data / name).write_text(name, encoding="utf-8")
    (legacy_data / "2busy1miss.lock").write_text("lock", encoding="utf-8")
    ...
    assert (target_data / "2busy1miss.sqlite3-wal").is_file()
    assert (target_data / "2busy1miss.sqlite3-shm").is_file()
